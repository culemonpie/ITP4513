...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	foreach ($_POST as $key=>$quantity){
		if ($quantity > 0){
			if (substr( $key, 0, 12 ) === "goodsNumber_"){
				...
				$goodsNumbers[$goodsNumber] = $quantity;
	...
	if ($is_valid){
		/*
		生成訂單
		扣貨 ( goods.remainingStock -= quantity)
		建立 orderItem (goodsID, quantity, orderID)
		Optional: 覆核訂單總價格
		*/
		...
		$error_message.="#3 - $qs<br>";

		mysqli_query($conn, $qs);
		$orderID = mysqli_insert_id($conn);
		foreach ($goodsNumbers as $goodsNumber => $quantity){
			// #4c - get selling price
			$qs = "select stockPrice from goods
			where goodsNumber = '$goodsNumber'
			limit 1";
			$error_message.="#4c - $qs<br>";
			$query = mysqli_query($conn, $qs) or die(mysqli_error($conn));
			$result = mysqli_fetch_assoc($query);
			$sellingPrice = $result["stockPrice"];
			$sum += $sellingPrice * $quantity;
			$error_message.="$sum / $sellingPrice / $quantity <br>";

			// #4 - add items to order
			$qs = "INSERT INTO orderItem(orderID, goodsNumber, quantity, sellingPrice) VALUES ('$orderID'
				,'$goodsNumber', '$quantity' , '$sellingPrice');";
				$error_message.="#4 - $qs<br>";
				$query = mysqli_query($conn, $qs) or die(mysqli_error($conn));

				// #2 - deduct inventory
				$qs = "UPDATE Goods SET remainingStock = remainingStock - $quantity where goodsNumber = '$goodsNumber';";
				$error_message.="#2 - $qs<br>";
				$query = mysqli_query($conn, $qs) or die(mysqli_error($conn));

				// #5 - update total price
				$qs = "UPDATE orders SET totalPrice = $sum where orderID = '$orderID';";
				$error_message.="#2 - $qs<br>";
				$query = mysqli_query($conn, $qs) or die(mysqli_error($conn));
			}
			...
			header("location: order-submitted.php?orderID=$orderID");
		}
	}
	...
	if (mysqli_num_rows($query) > 0){
		...
		<div class='row'>
		<div class='col-12 my-2'>
		<h4 class='text-secondary'>Store not found</h4>
		</div>
		</div>
		</div>
		");
